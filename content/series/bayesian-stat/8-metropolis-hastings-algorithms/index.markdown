...
{{< figure src="err_density.png" caption="Kernel density estimate of the errors in length." numbered="true" >}}
...
[^prior-density]: R code for slight biases in the normal density:
    ...
    ggsave("prior_density.png", width = 4, height = 3, dpi = 150)
    ```

{{< figure src="prior_density.png" caption="Kernel density estimate of the errors in length." numbered="true" >}}

We might prefer a prior on $\theta$ which captures our very strong bellief that the true bias is zero. One possibility as an alternative is the `Laplace distribution`:
...
{{< figure src="normal_vs_laplace.png" caption="Comparison of the Laplace prior to the normal prior." numbered="true" >}}
...
for (s in seq(2, burnin_steps + n_iter)) {
  for (i in seq(n_chains)) {
    ...
    r <- exp(
      -(1 / (2*sigma^2)) * sum((dat$error - theta_star)^2) -
      abs(theta_star) / b +
      (1 / (2*sigma^2)) * sum((dat$error - theta[s-1, i])^2) +
      abs(theta[s-1, i]) / b
    )
...
{{< figure src="metropolis_algo.png" caption="Trace plots and posterior density of $S=10000$ samples obtained using the Metropolis algorithm." numbered="true" >}}
...
{{< figure src="metropolis_algo_bad_init.png" caption="Trace plot and posterior density with bad initialization." numbered="true" >}}
